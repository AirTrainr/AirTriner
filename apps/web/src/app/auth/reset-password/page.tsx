    ...
    useEffect(() => {
        ...
        const handleReset = async () => {
            try {
                ...
                // Hash-aware grace period: extend the timeout when we can see a
                // recovery hash/code in the URL (slow networks may still be
                // exchanging it), and shorten it when the page was clearly
                // opened directly so the user isn't stuck on a spinner.
                const hash = window.location.hash || "";
                const looksLikeRecovery =
                    /access_token=|type=recovery|error_code=|error=/i.test(hash) || !!code;
                timeout = setTimeout(
